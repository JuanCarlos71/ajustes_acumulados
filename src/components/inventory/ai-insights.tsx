...
export function AiInsights({ analysisData }: AiInsightsProps) {
  ...
  const fetchInsights = useCallback(async () => {
    // Si no hay datos, no intentamos llamar a la IA
    ...
    try {
      ...
      // Llamada al Server Action (Genkit)
      ...
      // Mensaje específico para cuando falla dentro de Google Sites
      setError("Error de comunicación en Google Sites. Por favor, asegúrate de que el sitio esté publicado y recarga la página. Si el error persiste, verifica la API Key en Vercel.");
    } finally {
      setLoading(false);
    }
  }, [analysisData]);
  ...
  useEffect(() => {
    // Evitar múltiples llamadas al montar el componente
