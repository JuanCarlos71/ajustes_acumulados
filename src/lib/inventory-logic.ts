...
export function performAnalysis(
  ...
  const getVal = (obj: any, targetKeys: string[]) => {
    ...
    for (const target of targetKeys) {
      const found = objKeys.find(k => {
        ...
        return normalizedKey === normalizedTarget || k.toLowerCase().trim() === target.toLowerCase().trim();
      });
  ...
  return results.sort((a, b) => Math.abs(b.calculatedImpact || 0) - Math.abs(a.calculatedImpact || 0));
}
